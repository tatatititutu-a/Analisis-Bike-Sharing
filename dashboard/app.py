...
correlation_matrix = df.corr()  # Perbaikan: gunakan df, bukan df1
...
plt.figure(figsize=(10, 6))
plt.hist(df['cnt'], bins=20, color='skyblue', edgecolor='black')  # Perbaikan: gunakan df, bukan df1
...
plt.figure(figsize=(10, 6))
sns.boxplot(x='season', y='cnt', data=df, palette='Set3')  # Perbaikan: gunakan df, bukan df1
plt.xlabel('Season')
...
holiday_rental = df.groupby('holiday')['cnt'].mean()  # Perbaikan: gunakan df, bukan df1
...
weather_rental = df.groupby('weathersit')['cnt'].mean()  # Perbaikan: gunakan df, bukan df1
plt.figure(figsize=(8, 6))
...
plt.figure(figsize=(8, 6))
sns.boxplot(x='weathersit', y='cnt', data=df)  # Perbaikan: gunakan df, bukan df1
...
season_weather_rental = df.groupby('season_weather')['cnt'].mean()  # Perbaikan: gunakan df, bukan df1
